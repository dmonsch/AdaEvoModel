...
import adaevomodel.tools.experimental.pcm.IParametricRegression;
import adaevomodel.tools.experimental.pcm.noise.NormalDistributionNoise;
...
public class GeneralizationAwareRegression implements IParametricRegression {
	...
	public PCMRandomVariable performRegression(RegressionDataset dataset) {
		...
		PCMRandomVariable var = noiseGenerator.generateNoise(deviations, 100);
		...
		linearModelAndNoiseStoex.setSpecification(linearModelStoex.getSpecification() + " + " + var.getSpecification());
	...
	private double sampleFromRandomVariable(PCMRandomVariable var) {
		String spec = var.getSpecification();
		if (spec.startsWith("DoublePMF[")) {
			...
			while (mat.find()) {
				...
				double prob = Double.parseDouble(mat.group(2));
				currSum += prob;
				if (currSum >= rand) {
					return num;
				}
	...
	private void writeDataToFile(double[][] data) {
		...
		for (int i = 0; i < data.length; i++)// for each row
		{
			for (int j = 0; j < data[i].length; j++)// for each column
			{
				...
				if (j < data[i].length - 1)// if this is not the last row element
