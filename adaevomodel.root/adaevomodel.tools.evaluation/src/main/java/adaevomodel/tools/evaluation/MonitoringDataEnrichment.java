...
import adaevomodel.base.shared.pcm.util.PCMElementIDCache;
...
public class MonitoringDataEnrichment {
	...
	private PCMValidationPointMatcher pcmValidationPointMatcher = new PCMValidationPointMatcher();
	...
	private List<String> resolveServiceCallTargetIds(InMemoryPCM pcm, String hostId, String serviceId) {
		List<AllocationContext> matchingCtx = pcm.getAllocationModel().getAllocationContexts_Allocation().stream()
				.filter(ac -> {
					RepositoryComponent comp = ac.getAssemblyContext_AllocationContext()
							.getEncapsulatedComponent__AssemblyContext();
					if (comp instanceof BasicComponent) {
						BasicComponent bc = (BasicComponent) comp;
						return bc.getServiceEffectSpecifications__BasicComponent().stream().anyMatch(s -> {
							return s instanceof ResourceDemandingSEFF
									&& ((ResourceDemandingSEFF) s).getId().equals(serviceId);
						});
					}

					return false;
				}).collect(Collectors.toList());
		return matchingCtx.size() == 1
				? Lists.newArrayList(matchingCtx.get(0).getResourceContainer_AllocationContext().getId(), serviceId)
				: null;
	}
