package adaevomodel.runtime.pipeline.pcm.repository.core;
...
import adaevomodel.runtime.pipeline.pcm.repository.estimation.IResourceDemandTimeline;
import adaevomodel.runtime.pipeline.pcm.repository.estimation.ResourceDemandTimeline;
import adaevomodel.runtime.pipeline.pcm.repository.estimation.ResourceDemandTimelineInterval;
import adaevomodel.runtime.pipeline.pcm.repository.estimation.TimelineAnalyzer;
import adaevomodel.runtime.pipeline.pcm.repository.model.IResourceDemandEstimator;
import adaevomodel.runtime.pipeline.pcm.repository.tree.TreeNode;
import adaevomodel.runtime.pipeline.pcm.repository.usage.PlainUsageEstimation;
...
public class ResourceDemandEstimatorAlternative implements IResourceDemandEstimator {
	...
	public void prepare(MonitoringDataSet data) {
		...
		/*
		 * idMapping.entrySet().forEach(e -> { List<ResponseTimeRecord> recs =
		 * data.getResponseTimes().getResponseTimes(e.getKey()); if (recs != null) {
		 * recs.stream().forEach(r -> { if
		 * (r.getInternalActionId().equals("_o_OcwLQBEeq7laWsYbb_3A")) {
		 * TreeNode<ServiceCallRecord> current = e.getValue(); while (current != null &&
		 * !current.data.getCallerServiceExecutionId().equals("<not set>")) { if
		 * (current.parent == null) { System.out.println(current.data.getServiceId());
		 * System.out.println(current.data.getCallerServiceExecutionId()); }
		 * 
		 * current = current.parent; } if (current == null) {
		 * System.out.println("Inconsistent tree!"); } else { System.out.println("TEST"
		 * + current.data.getServiceId()); } } }); } });
		 */
	...
	public RepositoryStoexChanges derive(Map<String, Double> currentValidationAdjustment) {
		...
		TimelineAnalyzer analyzer = new TimelineAnalyzer(pcm, TimelineAnalyzer.UnrollStrategy.COMPLETE,
				new PlainUsageEstimation(), currentValidationAdjustment); // 20 s
