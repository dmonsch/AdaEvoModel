package adaevomodel.runtime.pipeline.pcm.repository.regression;
...
import adaevomodel.runtime.pipeline.pcm.repository.data.IfCondition;
import adaevomodel.runtime.pipeline.pcm.repository.data.IfDistribution;
...
public class ParametricLinearRegression {
	...
	private PCMRandomVariable deriveInner(List<Pair<ServiceParametersWrapper, Double>> inner) {
		...
		double lowestValue = inner.stream().mapToDouble(t -> t.getRight()).min().orElse(0d);
		...
		for (Pair<ServiceParametersWrapper, Double> tuple : inner) {
			...
			for (Entry<String, Object> parameter : tuple.getLeft().getParameters().entrySet()) {
				if (attributeIndexMapping.containsKey(parameter.getKey())) {
					...
					for (int i = 2; i <= MAX_DEPTH; i++) {
						...
						if (attributeIndexMapping.containsKey(attrName)) {
			...
			values[dataset.numAttributes() - 1] = (demand - lowestValue) * (1 + adjustmentFactor) + lowestValue;
	...
	private List<String> getDependentParameters(float thres, List<Pair<ServiceParametersWrapper, Double>> inner) {
		...
		PearsonsCorrelation spCorr = new PearsonsCorrelation();
